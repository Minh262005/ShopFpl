...
export async function register(TenTK, MatKhau, VaiTro = "user") {
  try {
    ...
    // Tạo MaTK mới
    const newId = (accounts.length > 0 ? (parseInt(accounts[accounts.length - 1].MaTK) + 1).toString() : "1");
...
export async function login(TenTK, MatKhau) {
  try {
    let res = await fetch(API_URL + `?TenTK=${encodeURIComponent(TenTK)}&MatKhau=${encodeURIComponent(MatKhau)}`);
    ...
    let accounts = JSON.parse(localStorage.getItem("TaiKhoan") || "[]");
    let user = accounts.find(acc => acc.TenTK === TenTK && acc.MatKhau === MatKhau);
    if (user) {
      saveSession(user);
