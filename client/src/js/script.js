...
async function loadAdminProducts() {
    ...
    try {
        ...
        products.forEach(product => {
            ...
            tr.innerHTML = `
                <td>${product.id ? product.id.substring(0, 6) + '...' : 'N/A'}</td>
                <td><img src="${product.image}" alt="${product.name}" style="width: 50px; height: 50px; object-fit: cover;"></td>
                <td>${product.name}</td>
                <td>₹${product.price.toFixed(2)}</td>
                <td>${product.category || ''}</td>
                <td>
                    <button class="btn small-btn" onclick="editProduct('${product.id}')">Edit</button>
                    <button class="btn small-btn delete-btn" onclick="deleteProduct('${product.id}')">Delete</button>
                </td>
            `;
...
async function loadMenuProducts(search = "") {
    const menuProductContainer = document.getElementById('menu-product-container');
    ...
    try {
        ...
        if (filtered.length === 0) {
            menuProductContainer.innerHTML = '<p style="text-align: center; color: var(--black);">No products found.</p>';
            return;
        }
        ...
        menuProductContainer.innerHTML = '<p style="text-align: center; color: red;">Cannot load products. Please check JSON Server!</p>';
    }
}
...
async function loadProductsForProductsSection() {
    try {
        ...
        if (productContainer) {
            productContainer.innerHTML = '<p style="text-align: center; color: red;">Không thể tải sản phẩm cho mục này.</p>';
        }
    }
}
...
function displayProducts(productsToDisplay, page) {
    ...
    if (paginatedProducts.length === 0) {
        productContainer.innerHTML = '<p style="text-align: center; color: var(--black);">Không có sản phẩm nào phù hợp.</p>';
        return;
    }
    ...
    paginatedProducts.forEach(product => {
        ...
        productBox.innerHTML = `
            <div class="icons">
                ...
                <a href="product-detail.html?id=${product.id}" class="fas fa-eye"></a>
            </div>
            <div class="image">
                <img src="${product.image}" alt="${product.name}">
            </div>
...
function setupProductFilters(productsSource) {
    ...
    const categories = ['all', ...new Set(productsSource.map(p => p.category).filter(Boolean))];
...
function filterAndDisplayProducts(productsSource) {
    ...
    if (currentCategory !== 'all') {
        filteredProducts = productsSource.filter(product => product.category === currentCategory);
    }
...
function removeCartItem(index) {
    ...
    if (document.getElementById("cart-items-table-body")) { // Nếu đang ở trang cart.html
...
async function createOrderFromCart(customerName) {
    ...
    try {
        ...
        alert("Đã xảy ra lỗi khi tạo đơn hàng. Vui lòng thử lại. Chi tiết lỗi: " + error.message);
    }
}
...
function showThankYouPage(orderId, customerName = "bạn") {
    ...
    const thankYouCustomerNameElement = document.getElementById("thank-you-customer-name");
    ...
    if (!thankYouSection) {
        ...
        alert(`Cảm ơn ${customerName}! Đơn hàng #${orderId} của bạn đã được đặt thành công.`);
    ...
    const cartPageContainer = document.querySelector(".cart-page .cart-table-container");
...
window.renderCartPage = () => {
    ...
    if (!cartTableBody || !cartSubtotalElement || !cartTotalElement || !proceedToCheckoutBtn || !customerNameInput) {
    ...
    const cartTableContainer = document.querySelector(".cart-page .cart-table-container");
    const cartSummary = document.querySelector(".cart-page .cart-summary");
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="quantity-controls">
                    ...
                    <input type="number" value="${item.quantity}" min="1" onchange="updateCartQuantityInput(${index}, this.value)">
                    <button onclick="updateCartQuantity(${index}, 1)">+</button>
                </div>
            </td>
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            if (username && password) {
                ...
                const user = accounts && accounts.find(acc => acc.TenTK === username && acc.MatKhau === password);
                if (user) {
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', (e) => {
            ...
            if (regUsername && regEmail && regPassword) {
                alert('Đăng ký thành công cho ' + regUsername + '! Bạn có thể đăng nhập ngay bây giờ.');
