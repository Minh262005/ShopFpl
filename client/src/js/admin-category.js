...
window.addCategory = async function (e) {
  ...
  if (
    categories.some((cat) => cat.TenDM.toLowerCase() === name.toLowerCase())
  ) {
  ...
  const newId =
    categories.length > 0
      ? (parseInt(categories[categories.length - 1].MaDM) + 1).toString()
      : "1";
...
window.updateCategory = async function (e) {
  ...
  if (
    categories.some(
      (cat) => cat.TenDM.toLowerCase() === name.toLowerCase() && cat.MaDM !== id
    )
  ) {
    alert("Category name already exists!");
    nameInput.focus();
  ...
  const catObj = categories.find((c) => c.MaDM === id);
  const apiId = catObj && catObj.id ? catObj.id : id;
  try {
    await fetch(`${API_URL}/${apiId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...catObj, MaDM: id, TenDM: name }),
    });
  } catch {
...
window.deleteCategory = async function (MaDM) {
  ...
  let categories = await getCategories();
  const catObj = categories.find((c) => c.MaDM === MaDM);
  const apiId = catObj && catObj.id ? catObj.id : MaDM;
  try {
    await fetch(`${API_URL}/${apiId}`, { method: "DELETE" });
  } catch {
  ...
  // Nếu đang ở chế độ sửa, reset form
  if (document.getElementById("category-id").value === MaDM) {
